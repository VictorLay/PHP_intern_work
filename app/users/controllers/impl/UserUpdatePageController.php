...
class UserUpdatePageController extends PermissionCtrl implements Controller
{
        ...
        if($this->isUserForUpdateNotAdmin()) {
            if ($this->checkPostKeys($this->requiredPostKeys) && $this->checkUserPermission()) {
                ...
                if ($userService->update($newUser)) {
                    ...
                    Redirection::redirect(SHOW_USER_PAGE . "?user_id=" . $newUser->getId());
                } else {
            ...
            Redirection::redirect(UPDATE_USER_PAGE);
        }
